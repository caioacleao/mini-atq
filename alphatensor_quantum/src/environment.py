...
class Environment:
  ...
  def init_state_with_basis(
      self,
      target_index: int | jt.Integer[jt.Array, 'batch'],
      change_of_basis: jt.Integer[jt.Array, '*batch size size'],
  ) -> EnvState:
    """Initializes states for an explicit target and change-of-basis matrix."""
    cob_matrix = jnp.asarray(change_of_basis, dtype=jnp.int32)
    if cob_matrix.ndim == 2:
      cob_matrix = cob_matrix[None]
    batch_size = cob_matrix.shape[0]

    target_indices = jnp.asarray(target_index, dtype=jnp.int32)
    if target_indices.ndim == 0:
      target_indices = jnp.full((batch_size,), target_indices, dtype=jnp.int32)
    if target_indices.shape[0] == 1 and batch_size != 1:
      target_indices = jnp.full(
          (batch_size,), target_indices[0], dtype=jnp.int32
      )
    if cob_matrix.shape[0] == 1 and target_indices.shape[0] != 1:
      cob_matrix = jnp.broadcast_to(
          cob_matrix,
          (target_indices.shape[0], self._config.max_tensor_size,
           self._config.max_tensor_size),
      )
      batch_size = target_indices.shape[0]

    init_tensors = self._target_tensors[target_indices]
    init_tensors_cob = jax.vmap(change_of_basis_lib.apply_change_of_basis)(
        init_tensors, cob_matrix
    )
    return EnvState(
        tensor=init_tensors_cob,
        past_factors=jnp.zeros(
            (
                batch_size,
                self._config.max_num_moves,
                self._config.max_tensor_size,
            ),
            dtype=jnp.int32,
        ),
        num_moves=jnp.zeros((batch_size,), dtype=jnp.int32),
        is_terminal=jnp.zeros((batch_size,), dtype=jnp.bool_),
        last_reward=jnp.zeros((batch_size,)),
        sum_rewards=jnp.zeros((batch_size,)),
        init_tensor_index=target_indices,
        change_of_basis=cob_matrix,
        factors_in_gadgets=jnp.zeros(
            (batch_size, self._config.max_num_moves), dtype=jnp.bool_
        ),
    )
