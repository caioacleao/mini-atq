...
class EnvironmentParams:
  ...
  """
  target_circuit_types: Sequence[tensors.TensorTarget]
...
class NetworkParams:
  ...
  Attributes:
    ...
    num_value_quantiles: Number of quantile outputs in the value head. A value
      of 0 keeps the original scalar value head.
    value_risk_quantile: Lower quantile threshold used to reduce quantile values
      to the scalar consumed by search.
    value_scalar_loss: Loss used by the scalar value head.
    value_huber_delta: Delta parameter for the scalar Huber loss.
    num_value_categorical_bins: Number of categorical value bins. A value of 0
      disables the categorical value head.
    value_support_min: Minimum return represented by the categorical value
      support.
    value_support_max: Maximum return represented by the categorical value
      support.
    value_target_transform: Optional invertible transform applied to the scalar
      value target before the (scalar) value loss. `none` keeps the original
      raw return target. `symlog` applies symlog(x) = sign(x) * log1p(|x|) to
      tame heavy-tailed returns; the scalar value head then predicts in symlog
      space and the inverse (symexp) is applied when the value is read back for
      search/decode, so the search-space scale is unchanged. Only affects the
      plain scalar value head (`num_value_quantiles == 0` and
