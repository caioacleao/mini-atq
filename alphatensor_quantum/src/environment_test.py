...
class EnvironmentTest(parameterized.TestCase):
  ...
  def test_init_state_with_basis(self):
    config = config_lib.EnvironmentParams(
        target_circuit_types=[tensors.CircuitType.SMALL_TCOUNT_3],
        max_num_moves=10,
        change_of_basis=config_lib.ChangeOfBasisParams(
    ...
    cob_matrix = jnp.array(
        [[1, 1, 0], [0, 1, 0], [1, 1, 1]], dtype=jnp.int32
    )
    env_state = env.init_state_with_basis(0, cob_matrix)
    ...
    with self.subTest('tensor'):
      np.testing.assert_array_equal(env_state.tensor[0], expected_tensor)
    with self.subTest('change_of_basis'):
      np.testing.assert_array_equal(env_state.change_of_basis[0], cob_matrix)
    with self.subTest('init_tensor_index'):
      np.testing.assert_array_equal(env_state.init_tensor_index, np.array([0]))
    with self.subTest('batch_shapes'):
      self.assertEqual(env_state.past_factors.shape, (1, 10, 3))
      self.assertEqual(env_state.factors_in_gadgets.shape, (1, 10))
