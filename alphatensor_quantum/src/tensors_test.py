...
class TensorsTest(parameterized.TestCase):
  ...
  def test_load_imported_targets_manifest(self):
    with tempfile.TemporaryDirectory() as tmpdir:
      tmpdir_path = Path(tmpdir)
      tensor_path = tmpdir_path / 'example.block1.tensor.npy'
      small_tensor = np.asarray(
          tensors.get_signature_tensor(tensors.CircuitType.SMALL_TCOUNT_3)
      )
      np.save(tensor_path, small_tensor.astype(bool))
      manifest_path = tmpdir_path / 'targets.json'
      manifest_path.write_text(json.dumps({
          'format': 'atq_imported_targets_v1',
          'targets': [{
              'name': 'example.block1',
      ...
      imported_targets = tensors.load_imported_targets_manifest(manifest_path)

      self.assertEqual(imported_targets[0].name, 'example.block1')
      self.assertEqual(
          imported_targets[0].tensor_path, str(tensor_path.resolve())
      )
      np.testing.assert_array_equal(
          tensors.get_signature_tensor(imported_targets[0]),
          small_tensor,
      )

  def test_validate_signature_tensor_rejects_non_symmetric_tensor(self):
    bad_tensor = np.zeros((3, 3, 3), dtype=np.int32)
    bad_tensor[0, 1, 2] = 1

    with self.assertRaisesRegex(ValueError, 'must be symmetric'):
      tensors.validate_signature_tensor(bad_tensor)
