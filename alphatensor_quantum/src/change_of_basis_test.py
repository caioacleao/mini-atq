...
class ChangeOfBasisTest(absltest.TestCase):
  ...
  def test_invert_matrix_gf2(self):
    cob_matrix = jnp.array([[1, 1, 0], [0, 1, 0], [1, 1, 1]], dtype=jnp.int32)
    inv_cob_matrix = change_of_basis.invert_matrix_gf2(cob_matrix)

    np.testing.assert_array_equal(
        jnp.mod(cob_matrix @ inv_cob_matrix, 2),
        jnp.eye(3, dtype=jnp.int32),
    )
    np.testing.assert_array_equal(
        jnp.mod(inv_cob_matrix @ cob_matrix, 2),
        jnp.eye(3, dtype=jnp.int32),
    )
